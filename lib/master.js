...
/**
 * This callback handles messages from the workers to the master process.
 * Depending on the content of the message, jobs get added or deleted from the queue.
 * Jobs are only added if they have not yet been queued.
 *
 * @param msg
 */
function workerCallback( msg ) {
	try {
		switch ( msg.replytype ) {
			...
			// Delete an item from the queue on error or success.
				// todo: Maybe change to use hashes instead of combinations of parameters to identify a job.
			case 'queue-del': {

				// Get the worker that has the job.
				let active_worker = arrWorkers.find( w => w.worker !== undefined && w.worker.process.pid === msg.workerid );

				active_worker.lastQueueDelete = new Date();
				...
				// Remove item from the queue and update totals.
				active_worker.site_queue.forEach( (job, i) => {
					if( job.job_key === msg.payload.job_key ) {
						active_worker.site_queue.splice( i, 1 );
						queuetotal--;
					}
				});
				...
				logger.debug("Deleted item from queue...");
