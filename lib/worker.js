...
const _url_analyzer = require('./url-analyzer')
...
function validateFileName( request ) {
	...
	if( request.screenWidth != request.width || request.screenHeight != request.height ) {
		suffix += '_screen' + request.screenWidth + 'x' + request.screenHeight;
	}
...
var HTTPListner = function() {
	...
	var routes = {
		...
		'/queue' : function( request, response ) {
			...
			if ( _get['url'] && _get['f'] ) {
				...
				if ( ! validateFileName( site ) ) {
					...
					_common.add_to_queue( _snapshot.get_job_key( site.url, site.file ), site );
				}
			} else {
		...
		'/analyze-url' : function( request, response ) {
			response.writeHead( 200, { 'Content-Type': 'text/plain' } );
			response.write("Analyze");
			response.end();

			const url = new URL( request.url, "http://example.com/" );
			const command = {
				url: url.searchParams.get( 'url' ),
				key: url.searchParams.get( 'cache_key' ),
				action: "analyze"
			}

			_common.add_to_queue( _url_analyzer.get_job_key( command.url ), command );

			process.send( { replytype: 'queue-add', workerid: process.pid, payload: command } );
		}
	}
