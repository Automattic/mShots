'use strict';
const analyze_url = require( "./url-analyzer" );
...
const _analyze_url = require( "./url-analyzer" );
...
function take_snapshot( p_uri, p_filename, p_width, p_height, screen_width, screen_height, scale_factor ) {
	let m_uri = p_uri;
	let job_key = get_job_key(p_uri, p_filename);
	...
	if ( ! parsedUri || ! parsedUri.host || ! parsedUri.protocol ) {
		...
		return _common.delete_from_queue( job_key, ( ! parsedUri.host ? HOST_INVALID : HOST_INVALIDSCHEMA ) );
	}
	...
	(async() => {
		const browser = await _common.get_browser();
		const page    = await browser.newPage();
		await page.setUserAgent( _common.default_UA + ' WordPress.com mShots' );
		...
		await page.on( 'request', request => {
			...
			_blacklist.allowHost( request.url(), function( result ) {
				if ( SUCCESS === result ) {
					...
					if ( ! request.url().match( /^(data:|https?:\/\/pixel.wp.com\/)/ ) ) {
		...
		if ( _twitter.isTweetEmbed( m_uri ) ) {
			...
			_common.delete_from_queue( job_key, success ? SUCCESS : GENERAL_ERROR );
			return;
		}
		...
		if ( response && response.ok() || page_loaded ) {
			...
			_common.delete_from_queue( job_key, SUCCESS );
			return;
		}
		...
		_common.delete_from_queue( job_key, ( 0 < block_all_code ? block_all_code : GENERAL_ERROR ) );
	})();
}
...
function get_job_key(url, filename) {
	return 'snapshot' + url + filename;
}
...
/**
 * Run a job from the queue if there is any, and the worker is not busy.
 */
function check_site_queue() {
	...
	let s_details = site_queue.shift();

	if ( s_details.action === 'analyze' ) {
		logger.debug( process.pid + ': analyzing: ' + s_details.url );
		_analyze_url.analyze( s_details.url, s_details.key );
	} else {
