	...
	(async() => {
		...
		if ( response && response.ok() || page_loaded ) {
			...
			if ( screen_width !== p_width || screen_height !== p_height ) {
				var clientSize = await page.evaluate(
					() => ( { clientWidth: document.documentElement.scrollWidth, clientHeight: document.documentElement.scrollHeight } ) );
				clientHeight = clientSize.clientHeight;
				clientWidth = clientSize.clientWidth;
			}
