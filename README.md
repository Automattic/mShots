...
- `npm install`: also configures (`./.env`) and builds the docker image (unless you pass `--ignore-scripts`)
- `npm start`: spins up the `dev` (docker) service in the `mshots-dev` container and starts the mshots (*nix) service on it
